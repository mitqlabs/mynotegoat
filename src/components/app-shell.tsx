...
/** Red pill badge for unread counts. Caps at 99+. */
function NavBadge({ count, className }: { count: number; className?: string }) {
  if (count <= 0) return null;
  return (
    <span
      className={classNames(
        "inline-flex min-w-[18px] items-center justify-center rounded-full bg-red-500 px-1.5 text-[10px] font-bold leading-[18px] text-white",
        className,
      )}
    >
      {count > 99 ? "99+" : count}
                      ...
                      {navItems.map((item) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {item.feature === "messages" && <NavBadge count={unreadMessages} />}
                          </Link>
                        );
                      })}
                    </nav>
                      ...
                      {navItems.map((item) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {item.feature === "messages" && unreadMessages > 0 && (
                              <NavBadge count={unreadMessages} className="absolute -right-1 -top-1" />
                            )}
                          </Link>
                        );
                      })}
